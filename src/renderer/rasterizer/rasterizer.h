	...
	while (vertex_id < vertex_offset + num_vertexes)
	{
		...
		for (auto& vertex : vertices)
		{
			...
			float2 bounding_box_begin{ 
				std::clamp(
				std::min(
					std::min(vertices[0].x, vertices[1].x), vertices[2].x), 0.f, static_cast<float>(width)-1.f),
			std::clamp(
				std::min(
					std::min(vertices[0].y, vertices[1].y), vertices[2].y), 0.f, static_cast<float>(height) - 1.f) };

			float2 bounding_box_end{ 
				std::clamp(
					std::max(
						std::max(vertices[0].x, vertices[1].x), vertices[2].x), 0.f, static_cast<float>(width) - 1.f),
			std::clamp(
				std::max(
					std::max(vertices[0].y, vertices[1].y), vertices[2].y), 0.f, static_cast<float>(height) - 1.f) };
			...
			for (int x = static_cast<int>(bounding_box_begin.x); x < static_cast<int>(bounding_box_begin.x); x++) 
			{
				for (int y = static_cast<int>(bounding_box_begin.y); y < static_cast<int>(bounding_box_begin.y); y++)
				{
					// bary for vertices[2]
