...
void cg::renderer::dx12_renderer::load_assets()
{
	...
	if (FAILED(device->CheckFeatureSupport(
		D3D12_FEATURE_ROOT_SIGNATURE, &rs_feature_data, 
	...
	rs_descriptor.Init_1_1(
		_countof(root_parameters), root_parameters, 0, 
		nullptr, rs_flags);
	...
	HRESULT result = D3DX12SerializeVersionedRootSignature(
		&rs_descriptor, rs_feature_data.HighestVersion, 
		&signature, &error);
	if (FAILED(result))
	{
	...
	THROW_IF_FAILED(vertex_buffer->Map(
		0, &read_range, 
