...
inline void raytracer<VB, RT>::build_acceleration_structure()
{
	for (auto& vertex_buffer : per_shape_vertex_buffer)
	{
		...
		{
			triangle<VB> triangle(vertex_buffer->item(vertex_id++), 
	...
	float3 position, float3 direction, float3 right, float3 up)
{
	for (int x = 0; x < width; x++)
	{
		for (int y = 0; y < height; y++)
		{
			...
			// comment anti-aliasing to pass tests
			//float u_delta = 1.f*x / static_cast<float>(width - 1) - 1.f;
			//u_delta *= static_cast<float>(width) / static_cast<float>(height);
			//float v_delta = 1.f*y / static_cast<float>(height - 1) - 1.f;
			...
			payload payload = trace_ray(ray_0, 1);

			ray ray_1(position, ray_direction + u_delta*right);
			payload payload = trace_ray(ray_1, 1);

			ray ray_2(position, ray_direction - v_delta*up);
			payload payload = trace_ray(ray_2, 1);

			ray ray_3(position, ray_direction + u_delta*right - v_delta*up);
			payload payload = trace_ray(ray_3, 1);

			cg::color accumed_color{ (payload_0.color.r +
									payload_1.color.1 +
									payload_2.color.r +
									payload_3.color.r) / 4.f,

									(payload_0.color.g +
									payload_1.color.g +
									payload_2.color.g +
									payload_3.color.g) / 4.f,

									(payload_0.color.b +
									payload_1.color.b +
									payload_2.color.b +
									payload_3.color.b) / 4.f };
			render_target->item(x, y) = RT::from_color(cg::color::from_float3(accumed_color));*/
		}
	}
}
