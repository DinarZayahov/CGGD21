...
struct unsigned_color
{
	static unsigned_color from_color(const color& color)
	{
		...
		out.g = std::clamp(
			static_cast<int>(255.f*color.g), 0, 255);
		out.b = std::clamp(
			static_cast<int>(255.f*color.b), 0, 255);
		return out;
	};
	float3 to_float3()
	{
		float3 ans;
		ans.x = static_cast<float>(this->r)/255.f;
		ans.y = static_cast<float>(this->g)/255.f;
		ans.z = static_cast<float>(this->b)/255.f;
